...
class Simple_Forum_Helper_Topic extends Mage_Core_Helper_Abstract
{
	...
	public function getAvLocale($locale)
	{
		...
		{
			if(!empty($this->editor_translate_keys[$short]))
			{
				return $this->editor_translate_keys[$short];
			}
			return $short;
		}
		else
	...
	public function getTopicsTreeArr($parent_id = 0, $status_all = true, $where = array(), $skip_parent = false)
	{
		...
		if(count($data) > 0)
		{
			...
			{
				$data[$key]['_childs'] = $this->getTopicsTreeArr($val['topic_id'], $status_all, $where);
			}
		}
		return $data;
	}

	public function getOptionsTopics($status_all = true, $skip_id = 0, $where = array(), $no_text = false, $skip_parent = false, $ret_as_it = false)
	{
		$ret            = array();
		...
		$arr = $this->getTopicsTreeArr(0, $status_all, $where, $skip_parent);
		$this->skip_id = $skip_id;
		if(!$ret_as_it)$ret = $this->addTopicsRecurcive($arr, $ret);
		else $ret = $this->addTopicsRecurciveArr($arr, $ret);
		return $ret;
	}

	private function addTopicsRecurciveArr($arr, $ret = array())
	{
		if(is_array($arr) && count($arr) > 0)
		{
			foreach($arr as $val)
			{
				if($val['topic_id'] == $this->skip_id)
				{
					continue;
				}
				if($val['is_category'] == 1 && is_array($val['_childs']) && count($val['_childs']) > 0)
				{
					$ret[] = array
					(
						'value'    => $this->addTopicsRecurciveArr($val['_childs']),
						'label'    => $val['title'],
	...
	private function addTopicsRecurcive($arr, $ret, $depth = 0)
	{
		if(is_array($arr) && count($arr) > 0)
		{
			foreach($arr as $val)
			{
				...
				(
					...
					'style'    => 'margin-left:' . $depth . 'px;' . ($depth == 0 ? 'font-weight:bold;' : ''),
					'url_text' => $val['url_text']
				);
				if(!empty($val['_childs']))
				{
					if(is_array($val['_childs']) && count($val['_childs']) > 0)
					{
						$ret = $this->addTopicsRecurcive($val['_childs'], $ret, $depth + 10);
					}
				}
			}
		}
	...
	public function validateUrlKey($key, $id)
	{
	...
	{
		$quantity  += $this->getChildsPosts($obj, $obj_parent)->getSize();
	...
	public function setUserForum($userId, $obj)
	{
		if(!$this->getUserBySystemId($userId))
		{
	...
	public function updateTotalPostsUser($system_user_id)
	{
		if(!$system_user_id)
		{
			return;
		}
		$o  = Mage::getModel('forum/user')->load($system_user_id, 'system_user_id');
		if($o->getId())
		{
	...
	public function getUserBySystemId($userId)
	{
		return Mage::getModel('forum/user')->load($userId, 'system_user_id')->getId();
	}

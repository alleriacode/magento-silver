	...
	private function _setLastPostInfo()
	{
		foreach($this->_objectsCollection as $key=>$val)
		{
			...
			if(!empty($obj[$val->getId()]))
			{
				...
				$this->_objectsCollection->getItemById($key)->setLatest_user_name(($obj[$val->getId()]['user_nick'] != '' && $obj[$val->getId()]['user_nick'] ? $obj[$val->getId()]['user_nick'] : $obj[$val->getId()]['user_name']));	

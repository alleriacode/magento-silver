...
class Simple_Forum_Block_Mytopics extends Mage_Core_Block_Template
{
	...
	public function initCollection()
	{
		...
		$this->_objectsCollection->getSelect()->where('is_category=0');
		if(!$isModerator )
		{
			$this->_objectsCollection->getSelect()->where('status=1');
			$this->_objectsCollection->getSelect()->where('system_user_id=?', $customerId);
		}
		elseif($status && $isModerator)
		{
			$status--;
			$this->_objectsCollection->getSelect()->where('status=?', $status);
		}

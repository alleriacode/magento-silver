     ...
     public function createAction()
      {
          $session = $this->_getSession();
           ...
           try  
           {
                ...
                $productData['short_description'] = $productdata1['description'];
              ...
              //  $productData['productcategory'] = $productdata1['productcategory'];
                $productData['downloadable_option'] =$productdata1['downloadableoption'];
          ...
          //echo "<pre>";print_r($productData);die('sdhjdfj');
                ...
                if($productdata1['specialproduct']=='48')
                {
                    ...
                    $dailydeal->setProductSku($prd->getSku());
                ...
                if($productdata1['producttype']=='ustream')
                    {
                        ...
                        $eventmodel->setEventContent($productdata1['description']);
                        $prefix=str_replace(" ","-",$productdata1['productname']);
                        $eventmodel->setEventUrlPrefix($prefix);
                  ...
                  /************** UPLOAD IMAGE   **********/ 
                    ...
                    $linkModel = Mage::getModel('downloadable/link')->setData(array(
                     ...
                     /********* send email to subscribed user ************/
                            ...
                            {
                                if($optiondata['value']== $productdata1['productexpertname']) {
                           ...
                           $resource = Mage::getSingleton('core/resource');
                            ...
                            $query="select * from tbl_brstmembersubscriber where member_id =". $productdata1['productexpertname'];
                            $result=$connection->query($query);
                            $custdata = $result->fetchAll(); 
                         
                            $i=0;
                            foreach($custdata as $datavalues) {
                         
                
                          $customer = Mage::getModel('customer/customer')->load($datavalues['customer_id']);
                          ...
                          $logo2               = $adminname;
                          $sender_name1         = $expertname;
                          

                          $mail_text1          = str_replace("{{ var customername}}", $firstname, $tem_text1);
                          $mail_text2          = str_replace("{{var expertname}}", $expertname, $mail_text1);
                           $mail_text3          = str_replace("{{var productname}}", $productdata1['productname'], $mail_text2);
                          //die('hello die here');
                          $to1        = $datavalues['customer_email'];
                          $headers   = 'MIME-Version: 1.0' . "\r\n";
                          $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
                              // Additional headers
                          $headers .= "To: $adminid" . "\r\n";
                          $headers .= 'From:'.$sender_name1.'<'.$sender_email1 .'>' . "\r\n";
                          mail($to1, $subject, $mail_text3, $headers);
                          $i++;
                            }
            ...
            try {
                ...
                {  
                   ...
                   // $productmodel->setSku($updatedata['productname']);
                    ...
                    $productmodel->setShortDescription($updatedata['description']); //added, meta description to 'short description' field, you can change this value

...
class FME_Events_Adminhtml_EventsController extends Mage_Adminhtml_Controller_Action
{
	...
	{//Mage::helper('events')->removeGalleryByEventId($this->getRequest()->getParam('id'));
		if ($data = $this->getRequest()->getPost())
		{
                   ...
                   // echo '<pre>';print_r($data);die('hjjsd');
			...
			if (isset($data['event_url_prefix']))
			{
				...
				{
					$isValid = false;
					$error .= 'The Url Prefix '.$data['event_url_prefix'].' already exists!';
				}
			}
			...
			{
				$isValid = false;
				$error .= 'Please attach only one product for an event';
			}
			...
			{//echo '<pre>';print_r($_FILES['event_image']);exit;
				...
				{	
					...
					$thumb_path = Mage::getBaseDir('media'). DS .'events'. DS .'thumbs';
				       $thumb_img = 'thumb_'.str_shuffle($_FILES['event_image']['name']);//echo $thumb_img;die('sdjks');
					$varImg->resize(135,135);
					$varImg->save($thumb_path, $thumb_img);
					$data['event_image'] = 'events/' .$_FILES['event_image']['name'];
					$data['event_thumb_image'] = 'events/thumbs/'.$thumb_img;
                                }
				catch (Exception $e)
				{

...
class Display_Allmembers_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
         $this->loadLayout();     
         $this->getLayout()->getBlock('head')->setTitle('Pferdeexperten');
	 $this->renderLayout();
    ...
    public function subscribeAction()
    {
          ...
          else 
          {
              ...
              $attr = $_product->getResource()->getAttribute("member_list");
               $adminmodel = Mage::getModel('admin/user')->getCollection()->getData();
            if ($attr->usesSource()): $expertname = $attr->getSource()->getOptionText($arrData['memberid']); endif;
           ...
           //  $customer = Mage::getModel("customer/customer")->setWebsiteId(Mage::app()->getWebsite()->getId())->loadByEmail($emailid);
